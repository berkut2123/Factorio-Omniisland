--[[--
Module for centralized initialization/loading.
Classes can register functions with this module to have them run when resetting globals or loading a savegame.
@module Init
@alias Initializer
]]
Init = {}
...
-- Initializer.globals = {}
function Init.funcs()
    if (global["_initFuncs"] == nil) then
        global["_initFuncs"] = Init.funcBackup
    end
...
-- function Init.reset()
--     for ind, f in pairs(Init.resetFuncs()) do
--         f()
--     end
-- end
...
function Init.doOnLoad()
    ...
    for ind, func in pairs(localFuncs) do
        func()
    end
end
