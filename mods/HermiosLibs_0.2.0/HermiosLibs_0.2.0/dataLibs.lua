ListPrototypesData={}
changing_keys={"name","place_result","result","icon","filename","placed_as_equipment_result"}
...
local function recursiveSetEntityAsInvisible(entityTable)
	for key,value in pairs (entityTable) do
		...
		--[[elseif key=="red" or key=="green" or key=="copper" or 
			(type(value)=="table" 
			and string.find(key,"offset") 
			and key~="offset_deviation") then
			entityTable[key]={0,0}]]--
...
local function recursiveCopyDataTable(oldTable,oldString,newString, parent_key)
	local newTable={}
	for k,d in pairs(oldTable) do		
		if not d then
			newTable[k]=nil
		elseif type(d)=="table" then			
			newTable[k]=recursiveCopyDataTable(d,oldString,newString, k) 
