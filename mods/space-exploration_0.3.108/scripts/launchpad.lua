    ...
    if not tick_task.land_timer then
      ...
      if is_landingpad then
        ...
        if math.random() < error_chance then
          -- off course
          is_landingpad = false
          local force = tick_task.launching_to_destiantion.landing_pad.container.force
          ...
          force.print({"space-exploration.rocket_survivability_fail", "[gps="..math.floor(new_pos.x)..","..math.floor(new_pos.y)..","..tick_task.launching_to_destiantion.zone.name.."]"})
        ...
        for item_type, item_count in pairs(tick_task.launched_contents) do
          if item_count > 1 then
            ...
            if lost_items > 0 then
              tick_task.launched_inventory.remove({name = item_type, amount = lost_items})
            end
          end
        end
      ...
      if tick_task.land_timer > Launchpad.time_landing_capsule_touchdown then
        ...
        if tick_task.launching_to_destiantion.landing_pad and tick_task.launching_to_destiantion.landing_pad.valid then
          ...
          if tick_task.launched_inventory and tick_task.launched_inventory.valid then
            ...
            local inv = tick_task.launching_to_destiantion.landing_pad.container.get_inventory(defines.inventory.chest)
...
--             tick_task.launching_to_destiantion.landing_pad.container.force.item_production_statistics.on_flow(Launchpad.name_rocket_section, reusable_parts)
          ...
          if reusable_parts < Launchpad.rocket_sections_per_rocket then
            tick_task.launching_to_destiantion.landing_pad.container.force.item_production_statistics.on_flow(Launchpad.name_rocket_section, -(Launchpad.rocket_sections_per_rocket - reusable_parts))
          end
          ...
          if tick_task.launched_inventory and tick_task.launched_inventory.valid then
            ...
            for i = 1, #tick_task.launched_inventory do
              for j = 0, #tick_task.safe_pods do
                ...
                local stack = tick_task.launched_inventory[i]
                if stack and stack.valid and stack.valid_for_read and stack.count then
                  Log.trace(tick_task.launched_inventory[i].name)
