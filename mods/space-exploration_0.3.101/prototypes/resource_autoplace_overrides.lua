...
local function resource_autoplace_settings(params)
  ...
  local frequency_multiplier = setting_scale(control_setting.frequency_multiplier)
  local size_multiplier = setting_scale(control_setting.size_multiplier)
  local size_boost = 4
  local density_multiplier = frequency_multiplier * size_multiplier

  local planet_control_setting = noise.get_control_setting("planet-size")
  ...
  local richness_post_multiplier = (params.richness_post_multiplier or 1) * setting_scale(control_setting.richness_multiplier)
  ...
  local starting_rq_factor = (params.starting_rq_factor_multiplier or 1) * 1 / 8
  ...
  local distance_affects_resources = tne(0) -- 0 to 1, 1 is where distance is a full factor
  local distance_affects_resources = noise.clamp(planet_control_setting.richness_multiplier, 0, 1) -- 0 to 1, 1 is where distance is a full factor
  -- note: Set planet-size richness to 0 for all non-homeworld zones.

  --local distance = noise.var("distance")
  local base_distance = 5000 -- (effective  assumed if real distance is not used)
  local distance = noise.clamp(base_distance + distance_affects_resources * (noise.var("distance") - base_distance), 0, base_distance) -- treat as permenant 5000
  ...
  -- Twinsen, August 2018
  local starting_resource_placement_radius = 140 --120
  ...
  local regular_patch_fade_in_distance = 320--300
  ...
  local starting_frequency_multiplier = ((frequency_multiplier - 1) * 0.25) + 1
  ...
  local minimum_favorability_for_full_placement = 1/4--1/2
  ...
  local regular_spots = tne{
    ...
    arguments =
    {
      ...
      suggested_minimum_candidate_point_spacing = tne(128),--tne(45.254833995939045), -- Magic number to match 0.17.50 spot placement, when candidate_point_count was always 128
      ...
      spot_radius_expression = litexp(size_boost + regular_spot_radius_expression),
  ...
  local starting_spots = tne{
    ...
    arguments =
    {
      ...
      candidate_spot_count = tne(64),--tne(32),
      suggested_minimum_candidate_point_spacing = tne(128), -- unset in vanilla
      ...
      spot_radius_expression = litexp(size_boost/2 + starting_rq_factor * starting_area_spot_quantity ^ (onethird)),
      spot_favorability_expression = litexp(1), --[[litexp( -- this seems to make the resources cluster a lot
        starting_feasibility * 2 -
        1 * distance / starting_resource_placement_radius +
        noise.random(0.5)
      ),]]
      basement_value = basement_value,
      maximum_spot_basement_radius = tne(64) -- does making this huge make a difference?
    }
  }
  ...
  local vein = 1 - 10 * noise.absolute_value(noise.function_application("factorio-multioctave-noise",
    {
      x = noise.var("x"),
      y = noise.var("y"),
      persistence = 0.5,
      seed0 = noise.var("map_seed"),
      seed1 = tne(seed1),
      input_scale = noise.fraction(1, 4), -- smaller 2nd value means smaller frequency
      output_scale = 1,
      octaves = 6,
    }
  ))
  local start_vein = 1 - 10 * noise.absolute_value(noise.function_application("factorio-multioctave-noise",
    {
      x = noise.var("x"),
      y = noise.var("y"),
      persistence = 0.5,
      seed0 = noise.var("map_seed"),
      seed1 = tne(seed1),
      input_scale = noise.fraction(1, 1), -- smaller 2nd value means smaller frequency
      output_scale = 1,
      octaves = 6,
    }
  ))

  --local regular_patches = regular_spots + ( 1 * blobs1f + 1.1 * vein * random_probability) * regular_blob_amplitude_expression
  local regular_patches = regular_spots + ( 1 * blobs1f + 0.8 * vein * random_probability) * regular_blob_amplitude_expression
  local starting_patches = starting_spots + ( 0.4 * blobs0f + 0.2 * start_vein * random_probability) * starting_blob_amplitude
