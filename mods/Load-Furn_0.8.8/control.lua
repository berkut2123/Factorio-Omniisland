...
local use_train = settings.global["loader-pro-use-trains"].value
...
local function getLoaderSearchData(wagon, direction)
  ...
  if direction == "west" then
    return {type = "loader-1x1", name = supported_loader_names, area = {{wagon.position.x+min_x-0.9, wagon.position.y+min_y+0.2}, {wagon.position.x+max_x-0.1, wagon.position.y+max_y-0.2}}} --was -1.5, -2.2, +0.5, +2.2
  elseif direction == "east" then
    return {type = "loader-1x1", name = supported_loader_names, area = {{wagon.position.x+min_x+1.1, wagon.position.y+min_y+0.2}, {wagon.position.x+max_x+0.9, wagon.position.y+max_y-0.2}}} --was +0.5, -2.2, +1.5, +2.2
  elseif direction == "north" then
    return {type = "loader-1x1", name = supported_loader_names, area = {{wagon.position.x+min_y+0.2, wagon.position.y+min_x-0.9}, {wagon.position.x+max_y-0.2, wagon.position.y+max_x-1.1}}} --was -2.2, -1.5, +2.2, -0.5
  elseif direction == "south" then
    return {type = "loader-1x1", name = supported_loader_names, area = {{wagon.position.x+min_y+0.2, wagon.position.y+min_x+1.1}, {wagon.position.x+max_y-0.2, wagon.position.y+max_x+0.9}}} --was -2.2, +0.5, +2.2, +1.5
  end
end
...
script.on_event(defines.events.on_runtime_mod_setting_changed, function(event)
    use_snapping = true
  if event.setting == "loader-pro-use-trains" then	--Check to make sure our setting has changed
    use_train = settings.global["loader-pro-use-trains"].value
    select_validator()
    global.wagons = {}
