...
function table.compare(tbl1, tbl2)
    ...
    for k, v in pairs(tbl1) do
        if type(v) == "table" and type(tbl2[k]) == "table" then
            if not table.compare(v, tbl2[k]) then
                return false
            end
        else
            if (v ~= tbl2[k]) then
...
function util.color(hex) -- supports 'rrggbb', 'rgb', 'rrggbbaa', 'rgba', 'ww', 'w'
    ...
    hex = hex:gsub("#", "")
    return #hex == 6 and {r = h(1, 2), g = h(3, 4), b = h(5, 6)} or #hex == 3 and {r = h(1), g = h(2), b = h(3)} or
        #hex == 8 and {r = h(1, 2), g = h(3, 4), b = h(5, 6), a = h(7, 8)} or
        #hex == 4 and {r = h(1), g = h(2), b = h(3), a = h(4)} or
        #hex == 2 and {r = h(1, 2), g = h(1, 2), b = h(1, 2)} or
        #hex == 1 and {r = h(1), g = h(1), b = h(1)} or
        {r = 1, g = 1, b = 1}
end
...
function util.add_shift_offset(offset_, table_)
    return util.foreach_sprite_definition(
        table_,
        function(tab)
...
util.insert_safe = function(entity, item_dict)
    if not (entity and entity.valid and item_dict) then
        return
    end
