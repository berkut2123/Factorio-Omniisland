...
local function gui_set_rule(player, type, index, element)
  ...
  if related ~= "" then
    ...
    if
      UPUtility.get_type(name) ~= UPUtility.get_type(related) and
        (not UPUtility.is_exception(UPUtility.get_type(name), UPUtility.get_type(related)))
     then
...
local function oc_import_config(event)
  ...
  if not textbox.type == "text-box" then
    return
  end
...
upgrade_planner_gui_events.register = function()
  ...
  Event.register(defines.events.on_gui_selection_state_changed, on_gui_selection_state_changed)
