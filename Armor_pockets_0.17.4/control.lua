	...
	if event.equipment.name == "armor-pocket-equipment" then	
		...
		if not armor.is_empty() then
			...
			if grid == event.grid then
				player.character_inventory_slots_bonus = inventorySlots + settings.startup["Armor_Pockets_SlotAmount"].value
			end
    	end
		...
		if not armor.is_empty() then
			...
			if grid == event.grid then		
		  		if event.count >= 0 then
			  		...
			  		local slotCount = inventory_slots_bonus - (settings.startup["Armor_Pockets_SlotAmount"].value * event.count)
		...
		if grid ~= nil then
			...
			for i = 1, #grid.equipment do
				...
				if grid.equipment[i].name == "armor-pocket-equipment" then
					total_slots = total_slots + settings.startup["Armor_Pockets_SlotAmount"].value
				end
