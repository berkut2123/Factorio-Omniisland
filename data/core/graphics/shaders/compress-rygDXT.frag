...
const float _1390[4] = float[](3.0, 0.0, 2.0, 1.0);
const int _1400[4] = int[](589824, 2304, 262402, 66562);
...
uvec2 stb_OptimizeColorsBlock()
{
    ...
    muv = (muv * 0.0625) + vec3(0.001953125);
...
bool stb_RefineBlock(inout uint pmax16, inout uint pmin16, uint mask)
{
    ...
    {
        vec3 c = vec3(0.0313725508749485015869140625);
        for (int i = 0; i < 16; i++)
        {
            c += block[i];
        }
        c *= 0.0625;
        ivec3 rgb = ivec3((c * 255.0) + vec3(0.5));
        int param = rgb.x;
        int param_1 = rgb.y;
        int param_2 = rgb.z;
...
uvec2 stb_CompressColorBlock()
{
    ...
    else
    {
        ...
        uint param_9 = min16;
        uint param_10 = mask;
        bool _1673 = stb_RefineBlock(param_8, param_9, param_10);
        max16 = param_8;
        min16 = param_9;
        if (_1673)
        {
...
void main()
{
    ...
    uvec2 _1720 = stb_CompressColorBlock();
    uvec2 colorBlock = _1720;
    fragColor = uvec4(alphaBlock.x, alphaBlock.y, colorBlock.x, colorBlock.y);
}
