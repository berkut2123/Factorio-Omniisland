...
local get_spawn_size_modifier = function()
...
function make_next_spawn_tick()
  ...
  local addition = math.ceil(script_data.random(interval[1], interval[2]) * get_spawn_size_modifier())
...
function get_wave_spawners()
  ...
  local wave_spawners = {}
  local count = math.min(#spawners, math.ceil(script_data.random(2, 10) * math.log(1 + script_data.day_number)))
  for k = count, 1, -1 do
...
local check_spawner_victory = function()
  local spawners = script_data.spawners
  for unit_number, spawner in pairs (spawners) do
    if spawner and spawner.valid then
      --There is at least 1 valid spawner, nothing to worry about.
      return
    else
      spawners[unit_number] = nil
    end
  end
  round_won()
end
...
function spawn_units()
  ...
  local set_command_to_unit = function(unit)
    local ai_settings = unit.ai_settings
    ai_settings.allow_try_return_to_spawner = false
    ai_settings.path_resolution_modifier = -1
    unit.speed = unit.prototype.speed * get_speed_multiplier()
  ...
  for k, spawner in pairs (some_spawns) do
    for k, unit in pairs (spawner.units) do
      ...
      set_command_to_unit(unit)
    end
  end

  while units_length > 0 do
    local k = random(units_length)
    local unit = units[k]
    local unit_name = unit.name
    local cost = prices[unit_name]
    local spawner_index = random(spawns_count)
    local random_spawner = some_spawns[spawner_index]
    local spawn_position = random_spawner.position
    local radius = (entities[unit_name].radius * 2) + 1 -- A bit of extra space for effectivity of pathing.
    for j = 1, math.min(unit.amount, random(30, 60)) do
      if cost > power then
        table.remove(units, k)
        units_length = units_length - 1
        break
      else
        local position = find_non_colliding_position(unit_name, spawn_position, 8, radius)
        if not position then
          --We can't fit any more biters into this base, remove it from the list.
          table.remove(some_spawns, spawner_index)
          spawns_count = spawns_count - 1
          if spawns_count == 0 then
            --We might have saturated every spawner, impressive. It's probably enough biters now, just return.
            return
          end
          break
        end
        local entity = create_entity{name = unit_name, position = position}
        set_command_to_unit(entity)
        power = power - cost
      end
    end
  end
...
function tell_stragglers_to_return()
  -- When a new day starts, we hope most units will be dead.
  -- Sometimes they will be stuck in the middle of a base, so we tell them to try to return to a spawner.
  local surface = script_data.surface
  if not (surface and surface.valid) then return end
  for k, unit in pairs (surface.find_entities_filtered{type = "unit", force = "enemy"}) do
...
local spawner_died = function(event)
  ...
  --If there are still valid spawners in this list, they haven't won
  local unit_number, next = next(script_data.spawners)
  if next and next.valid then return end

  --All spawners are dead, player wins!? Do one last double check!
  check_spawner_victory()
